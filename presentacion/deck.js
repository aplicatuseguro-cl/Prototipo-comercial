...
const eyebrow = (t,{x=M,y=0.92,color=INK3}={}) =>
  txt(t,x,y,CW,0.26,{size:10.5,bold:true,cs:2.4,color});
const title = (t,{x=M,y=1.36,w=CW,size=27,color=INK}={}) =>
  txt(t,x,y,w,2.0,{size,bold:true,color,lh:1.1});
...
const s1 = { bg:DARK, els:[
  ...
  txt('Todas tus coberturas de salud.\nFinalmente trabajando juntas.',M,8.25,CW,2.2,
      {size:32,bold:true,color:WHITE,lh:1.1}),
  regla(9.8),
  txt('Una forma de ayudar a los colaboradores y sus familias a entender y aprovechar mejor los beneficios de salud que la empresa ya financia.',
      M,10.25,7.9,1.3,{size:14.5,color:ICE,lh:1.34}),
...
const s3 = { bg:WHITE, els:[
  ...
  txt('Cada cobertura puede tener condiciones, topes, documentos y vigencias diferentes. El desafío aparece cuando una familia debe entenderlas y coordinarlas para una atención concreta.',
      M,2.9,CW,0.9,{size:14,lh:1.32}),
  ...p3.flatMap(([n,d],i)=>{ const y=4.2+i*1.5; return [
    rect(M,y,CW,0.02,{fill:LINE}),
    txt(n,M,y+0.32,2.85,0.6,{size:14.5,bold:true,color:NAVY,lh:1.16}),
...
VERBS.forEach(v=>{ const w=0.108*v.length+0.5;
  ...
  verbEls.push(txt(v,vx,4.72,w,0.42,{size:10,bold:true,cs:1.4,color:WHITE,align:'center',valign:'middle'}));
  vx+=w+0.16; });
...
const s4 = { bg:DARK, els:[
  ...
  txt('Es una propuesta de plataforma que ayuda a entender y coordinar las coberturas que una persona y su familia ya poseen.',
      M,2.9,7.9,0.9,{size:14.5,color:ICE,lh:1.34}),
  ...
  txt('EJEMPLO ILUSTRATIVO',M+0.5,CY+0.42,CW-1.0,0.26,{size:10,bold:true,cs:2.2,color:AMBER}),
  txt('La hija de María necesita\nuna resonancia de rodilla.',M+0.5,CY+0.82,CW-1.0,1.0,
      {size:21,bold:true,color:INK,lh:1.14}),
...
const s5 = { bg:WHITE, els:[
  ...
  ...p5.flatMap(([n,d],i)=>{ const y=4.0+i*1.85, h=1.55; return [
    ...
    txt(n,M+0.36,y+0.34,CW-0.72,0.34,{size:15,bold:true,color:NAVY}),
    txt(d,M+0.36,y+0.82,CW-0.72,0.6,{size:12.5,color:INK2,lh:1.3}),
  ];}),
  rrect(M,9.95,CW,2.2,{r:0.16,fill:DARK}),
  txt('TRANSPARENCIA',M+0.5,10.3,CW-1.0,0.26,{size:10,bold:true,cs:2.2,color:STEEL}),
...
const s6 = { bg:WHITE, els:[
  eyebrow('PRIVACIDAD'),
  title('El diseño propuesto separa la experiencia\nindividual del dashboard corporativo.',{size:24}),
  // lo que no se comparte
  rrect(M,3.3,CW,2.8,{fill:BG2,line:LINE}),
  txt('LA EMPRESA NO ACCEDE A',M+0.36,3.59,CW-0.72,0.26,{size:10,bold:true,cs:2,color:AMBER}),
  ...PN.flatMap((t,i)=>[ell(M+0.38,4.23+i*0.33,0.1,AMBER),
       txt(t,M+0.64,4.13+i*0.33,CW-1.0,0.3,{size:12,color:INK2})]),
  // lo que sí podría recibir
  rrect(M,6.45,CW,2.55,{fill:BG2,line:LINE}),
  txt('LA EMPRESA PODRÍA RECIBIR',M+0.36,6.74,CW-0.72,0.26,{size:10,bold:true,cs:2,color:GREEN}),
  ...PS.flatMap((t,i)=>[ell(M+0.38,7.38+i*0.33,0.1,GREEN),
       txt(t,M+0.64,7.28+i*0.33,CW-1.0,0.3,{size:12,color:INK2})]),
  // el mismo registro, dos miradas
  txt('El mismo registro, dos miradas',M,9.45,5,0.3,{size:15,bold:true,color:INK}),
  txt('Atención completamente simulada',M+4.0,9.49,CW-4.0,0.28,{size:11,color:INK3,align:'right'}),
  rrect(M,10.0,BW,1.75,{fill:WHITE,line:LINE}),
  txt('COLABORADOR',M+0.28,10.26,BW-0.56,0.24,{size:9.5,bold:true,cs:1.6,color:NAVY}),
  ...REG.flatMap(([k,v],i)=>[
    txt(k,M+0.28,10.7+i*0.3,1.4,0.24,{size:10,color:INK3}),
    txt(v,M+1.62,10.7+i*0.3,BW-1.9,0.24,{size:10,color:INK,bold:true})]),
  rrect(M+BW+0.3,10.0,BW,1.75,{fill:WHITE,line:LINE}),
  txt('RECURSOS HUMANOS',M+BW+0.58,10.26,BW-0.56,0.24,{size:9.5,bold:true,cs:1.6,color:NAVY}),
  ...REGX.flatMap(([k,v],i)=>[
    txt(k,M+BW+0.58,10.7+i*0.3,1.4,0.24,{size:10,color:INK3}),
    txt(v,M+BW+1.92,10.7+i*0.3,BW-1.9,0.3,{size:10,color:INK3,lh:1.14})]),
  nota('Los umbrales y las medidas definitivas de anonimización deben validarse antes de un piloto. El ejemplo no representa una garantía legal o técnica.',{y:12.2}),
